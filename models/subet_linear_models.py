...
df = pd.read_parquet('./data/subset_imputed.parquet')
...
categorical_feats = ['country_text_id']
...
X_ols['country_text_id'] = X_ols['country_text_id'].fillna('missing')
X_ols = pd.get_dummies(X_ols, columns=['country_text_id'], drop_first=True)
...
plt.ylim(0, 1)
plt.tight_layout()
plt.savefig("./writeup/plots/benchmark_sub_linear.png", dpi=300)
...
plt.tight_layout()
plt.savefig("./writeup/plots/benchmark_sub_linear_scatter.png", dpi=300)
