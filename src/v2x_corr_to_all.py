# this script runs the correlation test of all the vars against our target
# v2x_corr
import pandas as pd
...
# 8. Save all variables in descending correlation
df_all_corr = (
    corr_with_pubcorr
    .sort_values(ascending=False)
    .reset_index()
    .rename(columns={"index": "variable", keep_index: "correlation"})
)
df_all_corr.to_csv("./data/all_correlations_desc.csv", index=False)
print("Wrote all correlations (desc) to all_correlations_desc.csv")

# 9. Extract top‐10 positive and top‐10 negative correlations
top_pos = corr_with_pubcorr.nlargest(10)
top_neg = corr_with_pubcorr.nsmallest(10)

# 10. Convert to DataFrame for easy viewing/output
df_pos_top_10 = (
    top_pos
    .reset_index()
    .rename(columns={"index": "variable", keep_index: "correlation"})
...
# 11. Display or save your top lists
